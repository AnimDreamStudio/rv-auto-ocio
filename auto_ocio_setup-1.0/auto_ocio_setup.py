...
def read_auto_ocio(*args, **kwargs):
    """
    Read auto ocio config file.

    """
    # get auto ocio config file path.
    auto_ocio_cfg = "{}/ocio.config".format(root_path)
    if os.path.exists(auto_ocio_cfg):
        return {}
    else:
        with open(auto_ocio_cfg, "r") as f:
            data = json.load(f)
    return data
